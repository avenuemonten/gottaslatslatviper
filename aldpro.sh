#!/bin/bash
# ффункция информирования о ходе установки скрипта. Первый аргумент - какой этап скрипта выполняется
info () {
        echo ">-----Выполняется: $1"
        echo -e "\n---###############---Выполняется: $1\n" >> .log
        sleep 1
...
# добавляем репоизтории ALDPRO
        ...
        echo "deb https://dl.astralinux.ru/aldpro/stable/repository-main/ 2.1.0 main" >> /etc/apt/sources.list.d/aldpro.list
        echo "deb https://dl.astralinux.ru/aldpro/stable/repository-extended/ generic main" >> /etc/apt/sources.list.d/aldpro.list
